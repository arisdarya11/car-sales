...
.block-container {{ padding: 1.4rem 2rem 3rem !important; max-width: 1440px; }}
...
/* Selectbox & slider inside filter bar */
...
.kpi-card:hover {{ box-shadow: 0 4px 12px rgba(26,23,20,0.09); }}
.kpi-card::after {{
    content: '';
    position: absolute;
...
.kpi-label  {{ font-size:0.63rem; font-weight:600; text-transform:uppercase; letter-spacing:0.1em; color:{INK2}; margin-bottom:5px; }}
.kpi-value  {{ font-family:'JetBrains Mono',monospace; font-size:1.65rem; font-weight:500; color:{INK}; line-height:1; margin-bottom:4px; }}
.kpi-sub    {{ font-size:0.68rem; color:{INK2}; }}
...
.insight-pill {{
    ...
    padding:9px 14px; font-size:0.73rem; color:{INK2};
...
.chart-title {{ font-size:0.8rem; font-weight:600; color:{INK}; margin-bottom:2px; letter-spacing:-0.01em; }}
.chart-sub   {{ font-size:0.68rem; color:{INK2}; margin-bottom:10px; }}

/* ── RANK BARS ── */
.rank-row {{
    display:grid; grid-template-columns:110px 1fr 55px;
    align-items:center; gap:8px; margin-bottom:8px; font-size:0.74rem;
}}
...
.dash-header {{
    ...
    padding-bottom:1rem; border-bottom:1px solid {BORDER};
}}
.logo-eyebrow {{ font-family:'JetBrains Mono',monospace; font-size:0.63rem; letter-spacing:0.14em; text-transform:uppercase; color:{ACCENT}; margin-bottom:3px; }}
.logo-title   {{ font-size:1.55rem; font-weight:700; color:{INK}; letter-spacing:-0.03em; line-height:1; }}
.logo-title span {{ color:{ACCENT}; }}
.logo-sub     {{ font-size:0.74rem; color:{INK2}; margin-top:3px; }}
.hstats       {{ display:flex; gap:18px; align-items:center; flex-wrap:wrap; }}
.hstat-val    {{ font-family:'JetBrains Mono',monospace; font-size:1rem; font-weight:500; color:{INK}; }}
.hstat-label  {{ font-size:0.63rem; color:{INK2}; text-transform:uppercase; letter-spacing:0.08em; }}
...
<div class="dash-header">
  <div>
    ...
    <div class="logo-sub">Car Sales Market Intelligence · 157 Models · 30 Brands</div>
  </div>
  <div class="hstats">
    <div><div class="hstat-val">{df_raw["Sales_in_thousands"].sum()/1000:.1f}M</div><div class="hstat-label">Total Units</div></div>
    <div><div class="hstat-val">${df_raw["Price_in_thousands"].mean():.1f}K</div><div class="hstat-label">Avg. Price</div></div>
    <div><div class="hstat-val">{df_raw["Horsepower"].mean():.0f} hp</div><div class="hstat-label">Avg. Power</div></div>
    <div><div class="hstat-val">{df_raw["Fuel_efficiency"].mean():.1f} mpg</div><div class="hstat-label">Avg. MPG</div></div>
  </div>
</div>
""", unsafe_allow_html=True)
...
with st.container():
    ...
    with fc1:
        ...
        sel_seg  = st.selectbox("seg", seg_opts, label_visibility="collapsed", key="seg")

    with fc2:
        st.markdown("<div class='filter-label'>Vehicle Type</div>", unsafe_allow_html=True)
    ...
    with fc3:
        ...
        sel_mfrs = st.multiselect("mfr", all_mfrs, default=all_mfrs,
                                  label_visibility="collapsed", key="mfr",
                                  placeholder="All manufacturers…")

# fixed defaults — no price/sales sliders
price_max = 53
min_sales = 0
...
st.markdown(f"""
<div style="margin:-8px 0 14px; padding: 0 4px; font-size:0.7rem; color:{INK2};">
  <b style="color:{INK};">{n}</b> of {n_total} models
  {"· " + chips_html if chips else " · <span style='color:" + GREEN + ";font-weight:600'>All filters default</span>"}
</div>
""", unsafe_allow_html=True)
...
with col_left:
    ...
    brand_df   = (df.groupby("Manufacturer")["Sales_in_thousands"].sum()
    ...
    fig_brand = go.Figure(go.Bar(
        x=brand_df["Sales_in_thousands"], y=brand_df["Manufacturer"],
        orientation="h",
        ...
        hovertemplate="<b>%{y}</b><br>Sales: %{x:,.0f}K<extra></extra>",
    ))
...
with col_right:
    ...
    fig_seg = go.Figure(go.Pie(
        labels=segs, values=seg_vals, hole=0.62,
    ...
    fig_seg.add_annotation(
        ...
        x=0.5, y=0.5, showarrow=False, font=dict(color=INK, size=14), align="center",
    )
    ...
    fig_sp = go.Figure(go.Bar(
        ...
        textposition="outside", textfont=dict(size=9, color=INK2),
        hovertemplate="<b>%{x}</b><br>Avg Price: $%{y:.1f}K<extra></extra>",
    ))
    fig_sp.update_layout(
        **plot_layout(height=140, margin=dict(l=8, r=8, t=28, b=8)),
...
# ═════════════════════════════════════════════════════════════
# ROW 2 — Top Models | Price vs Sales | HP vs MPG
# ═════════════════════════════════════════════════════════════
...
with c1:
    ...
    fig_vt = go.Figure(go.Pie(
        labels=vtype_sales["Vehicle_type"], values=vtype_sales["Sales_in_thousands"],
        hole=0.55, marker=dict(colors=vtype_colors, line=dict(color=SURFACE, width=2)),
...
with c2:
    ...
    for seg, clr in seg_color_map.items():
        ...
        fig_scat.add_trace(go.Scatter(
            x=sub["Price_in_thousands"], y=sub["Sales_in_thousands"],
            mode="markers", name=str(seg),
            marker=dict(size=np.sqrt(sub["Horsepower"]) * 0.85,
    ...
    if xl:
        fig_scat.add_trace(go.Scatter(x=xl, y=yl, mode="lines",
    ...
    for seg, clr in seg_color_map.items():
        ...
        fig_ew.add_trace(go.Scatter(
            x=sub["Engine_size"], y=sub["Curb_weight"], mode="markers", name=str(seg),
...
with c3:
    ...
    for seg, clr in seg_color_map.items():
        ...
        fig_eff.add_trace(go.Scatter(
            x=sub["Horsepower"], y=sub["Fuel_efficiency"],
            mode="markers", name=str(seg),
    ...
    if xl:
        fig_eff.add_trace(go.Scatter(x=xl, y=yl, mode="lines",
            line=dict(color=hex_to_rgba(ACCENT, 0.4), width=1.8, dash="dot"),
            showlegend=False, hoverinfo="skip"))
    ...
    for seg, clr in seg_color_map.items():
        ...
        fig_ppf.add_trace(go.Box(
            y=sub["Power_perf_factor"], name=str(seg),
            marker_color=clr, line=dict(color=clr, width=1.5),
            fillcolor=hex_to_rgba(clr, 0.15), boxmean=True,
...
# ═════════════════════════════════════════════════════════════
# ROW 3 — Resale + Correlation heatmap
# ═════════════════════════════════════════════════════════════
...
with r3a:
    ...
    fig_res = go.Figure(go.Bar(
        ...
        textposition="outside", textfont=dict(size=9, color=INK2),
        hovertemplate="<b>%{y}</b><br>Retains %{x:.1f}%<extra></extra>",
    ))
...
with r3b:
    ...
    st.markdown('<div class="chart-sub">Pearson correlations · Red = negative, Blue = positive</div>', unsafe_allow_html=True)
    ...
    fig_heat = go.Figure(go.Heatmap(
        ...
        zmid=0, text=corr_mat.values,
        texttemplate="%{text:.2f}", textfont=dict(size=9, color=INK),
    ...
    corr_with_sales = corr_mat["Sales_in_thousands"].drop("Sales_in_thousands")
    corr_sorted     = corr_with_sales.reindex(corr_with_sales.abs().sort_values(ascending=False).index)
    corr_colors_bar = [GREEN if v > 0 else ACCENT for v in corr_sorted.values]
    labels_map      = dict(zip(num_cols[1:], corr_labels[1:]))
    nice_labels     = [labels_map.get(i, i) for i in corr_sorted.index]

    fig_corr_bar = go.Figure(go.Bar(
        x=nice_labels, y=corr_sorted.values,
        marker=dict(color=[hex_to_rgba(c, 0.8) for c in corr_colors_bar], line=dict(width=0)),
        text=[f"{v:+.2f}" for v in corr_sorted.values],
        textposition="outside", textfont=dict(size=9, color=INK2),
    ...
    yspan = max(abs(corr_sorted.values))
    fig_corr_bar.update_yaxes(range=[-yspan * 1.45, yspan * 1.45])
...
tbl = df.copy()
if search:
    mask = (tbl["Model"].str.contains(search, case=False, na=False) |
